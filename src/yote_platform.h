...
#define PLATFORM_RECEIVE_FROM(name)                                                                    \
    i32 name(PlatformSocket sock, void* buffer, i32 size, u32* from_ip, u16* from_port)
typedef PLATFORM_RECEIVE_FROM(platform_receive_from);

#define PLATFORM_SEND_TO(name)                                                                         \
    i32 name(PlatformSocket sock, void* buffer, i32 size, u32 to_ip, u16 to_port)
...
PLATFORM_BUFFER_WRITE_TO_FILE(platform_win_buffer_write_to_file) {
    HANDLE file_handle =
        CreateFileA(file_path, FILE_APPEND_DATA, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
PLATFORM_BUFFER_LOAD_FROM_FILE(platform_win_buffer_load_from_file) {
    HANDLE file_handle =
        CreateFileA(file_path, GENERIC_READ, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
...
PLATFORM_RECEIVE_FROM(platform_win_receive_from) {
    ...
    int result =
        recvfrom(sock.socket, buffer, size, 0, (struct sockaddr*)&from_address, &from_address_size);
...
PLATFORM_SEND_TO(platform_win_send_to) {
    ...
    i32 result = sendto(sock.socket, buffer, size, 0, (SOCKADDR*)&to_address, sizeof(to_address));
